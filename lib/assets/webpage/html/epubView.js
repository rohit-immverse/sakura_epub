...
function applyHeadingFontSizes(contents, fontSize) {
  try {
    ...
    style.textContent = `
      ...
      td { font-size: ${fontSize}px !important; }
      th { font-size: ${fontSize}px !important; }
      blockquote {
...
function makeTablesResponsive(contents) {
  try {
    ...
    let tableFontSize = 12; // fallback
    try {
      const currentFont = parseInt(
        window.getComputedStyle(doc.documentElement).fontSize || "16",
        10
      );
      tableFontSize = Math.max(currentFont - 4, 10); // don't go too tiny
    } catch (e) {
      tableFontSize = 12;
    }
