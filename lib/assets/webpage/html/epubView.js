...
function applyHeadingFontSizes(contents, fontSize) {
  try {
    const doc = contents.document;
    if (!doc) return;

    let style = doc.getElementById('epub-heading-fontsize-style');
    if (style) style.remove();

    style = doc.createElement('style');
    style.id = 'epub-heading-fontsize-style';
    style.textContent = `
      h1 { font-size: ${fontSize + 5}px !important; }
      h2 { font-size: ${fontSize + 4}px !important; }
      h3 { font-size: ${fontSize + 3}px !important; }
      h4 { font-size: ${fontSize + 2}px !important; }
      h5 { font-size: ${fontSize + 1}px !important; }
      h6 { font-size: ${fontSize}px !important; }
      shloka { font-size: ${fontSize}px !important; }
      td { font-size: ${fontSize - 2}px !important; }
      th { font-size: ${fontSize - 2}px !important; }
      blockquote {
    ...
    console.error('Error applying heading font sizes:', e);
  }
}
...
function loadBook(data, cfi, initialXPath, manager, flow, spread, snap, allowScriptedContent, direction, useCustomSwipe, backgroundColor, foregroundColor, fontSize, clearSelectionOnNav, selectAnnotationRangeParam, customCss) {
  ...
  _currentFontSize = Number(fontSize) || 16;
  rendition.hooks.content.register(function (contents) {
    if (_currentFontFamily) {
      _injectFontCSS(contents, _currentFontFamily, _currentFontBase64, _currentFontMimeType);
    }

    applyLongWordFix(contents);
    applyHeadingFontSizes(contents, _currentFontSize);
    makeTablesResponsive(contents);
    disableTextSelection(contents);
  });
  ...
  rendition.on("relocated", function (location) {
    ...
    Promise.all([
      ...
      var locationData = {
        ...
        endXpath: xpaths[1],
        progress: percent,
        page: location.start.displayed ? location.start.displayed.page : null,
        totalPages: location.start.displayed ? location.start.displayed.total : null
      }
      ...
      rendition.getContents().forEach(function (contents) {
        try {
          pagesHtml.push(contents.document.documentElement.outerHTML);
        } catch (e) {
          console.error('Failed to get page HTML:', e);
        }
      });

      window.flutter_inappwebview.callHandler('relocated', {
  ...
  rendition.hooks.content.register((contents) => {
    ...
    doc.addEventListener("click", function (event) {
      ...
      if (["a", "span", "h2"].includes(tag)) {
        ...
        console.log("Clicked tag:", clickedInfo);

        callFlutterHandler("tagClicked", clickedInfo);
      }
    });
...
function disableTextSelection(contents) {
  try {
    const doc = contents.document;
    if (!doc) return;

    let style = doc.getElementById('epub-disable-selection-style');
    if (style) style.remove();

    style = doc.createElement('style');
    style.id = 'epub-disable-selection-style';
    style.textContent = `
      html, body,
      p, div, span, li, a, blockquote,
      h1, h2, h3, h4, h5, h6,
      td, th, pre, code {
    ...
    console.error('Error disabling text selection:', e);
  }
}

function makeTablesResponsive(contents) {
  try {
    const doc = contents.document;
    if (!doc) return;

    // Get current rendition font size
    let tableFontSize = 14; // fallback
    try {
      const currentFont = parseInt(
        window.getComputedStyle(doc.documentElement).fontSize || "16",
        10
      );
      tableFontSize = Math.max(currentFont - 2, 10); // don't go too tiny
    } catch (e) {
      tableFontSize = 14;
    }

    // Add / replace style
    let style = doc.getElementById('epub-table-responsive-style');
    if (style) style.remove();

    style = doc.createElement('style');
    style.id = 'epub-table-responsive-style';
    style.textContent = `
      .epub-table-scroll {
        width: 100% !important;
    ...
    // Wrap each table only once
    const tables = doc.querySelectorAll('table');
    tables.forEach((table) => {
      if (
        table.parentElement &&
        table.parentElement.classList.contains('epub-table-scroll')
      ) {
        return;
      }

      const wrapper = doc.createElement('div');
      wrapper.className = 'epub-table-scroll';
      table.parentNode.insertBefore(wrapper, table);
      wrapper.appendChild(table);
    });
  } catch (e) {
    console.error('Error making tables responsive:', e);
  }
}

function applyLongWordFix(contents) {
  try {
    const doc = contents.document;
    if (!doc) return;

    if (doc.getElementById('epub-long-word-fix-style')) return;

    const style = doc.createElement('style');
    style.id = 'epub-long-word-fix-style';
    style.textContent = `
      html, body,
      p, div, span, li, a, blockquote,
      td, th, pre, code {
    ...
    console.error('Error applying long word fix:', e);
  }
}
...
function next() {
  ...
  if (!rendition || _navInProgress) return;

  _navInProgress = true;

  try {
    var viewer = document.getElementById('viewer');
    if (viewer) viewer.style.opacity = '0.98';

    var navPromise = rendition.next();
...
function previous() {
  ...
  try {
    // Optional: hide tiny render flicker during prev transition
    var viewer = document.getElementById('viewer');
    if (viewer) {
      viewer.style.opacity = '0.98';
    }

    var navPromise = rendition.prev();
    ...
    var viewer = document.getElementById('viewer');
    if (viewer) viewer.style.opacity = '1';
    console.error('Error in previous():', e);
  }
}
...
//   rendition.themes.fontSize(`${_currentFontSize}px`);
...
//     });
//   } catch (e) {
//     console.error('Error updating heading font sizes:', e);
//   }
...
function setFontSize(fontSize) {
  ...
  // Save current visible location before layout changes
  var currentCfi = null;
  try {
    if (rendition.location && rendition.location.start && rendition.location.start.cfi) {
      currentCfi = rendition.location.start.cfi;
    }
  } catch (e) {
    console.error('Error reading current location before font resize:', e);
  }

  // Apply font size
  rendition.themes.fontSize(`${fontSize}px`);

  // Also re-apply heading sizes if you already added that helper
  try {
    if (typeof applyHeadingFontSizes === 'function') {
      rendition.getContents().forEach(function (contents) {
        applyHeadingFontSizes(contents, fontSize);
      });
    }
  } catch (e) {
    console.error('Error applying heading sizes after font resize:', e);
  }

  // Force epub.js to recompute layout / columns
  setTimeout(function () {
    try {
      // Trigger viewport recalculation
      if (rendition.manager && typeof rendition.manager.resize === 'function') {
        rendition.manager.resize(window.innerWidth, window.innerHeight);
      } else if (typeof rendition.resize === 'function') {
        rendition.resize(window.innerWidth, window.innerHeight);
      }

      // Re-display current location after resize so blank page is removed
      if (currentCfi) {
        rendition.display(currentCfi);
      }

      // Send updated location back to Flutter
      setTimeout(function () {
        rendition.reportLocation();
      }, 30);
    } catch (e) {
      console.error('Error refreshing layout after font size change:', e);
    }
  }, 80);
}
